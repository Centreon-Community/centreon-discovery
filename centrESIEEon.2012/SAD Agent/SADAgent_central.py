...
def scanRangeIP(db):
	...
	c.execute("""SELECT R.id, S.ns_ip_address, R.plage, R.cidr\
			 FROM nagios_server S, sad_rangeip R\
...
def checkStatusPoller(poller):
	...
	try:
		...
		if rcv.startswith("#reply#"):
			req = "UPDATE sad_rangeip SET poller_status = %d WHERE id=%d" % (1, poller[ID])
			c.execute(req)
		s.close()
	except socket.error:
		req = "UPDATE sad_rangeip SET poller_status = %d WHERE id=%d" % (2, poller[ID])
		c.execute(req)
...
def getHostsFromPoller(poller):
	...
	try:
		...
		req = "DELETE FROM sad_results WHERE plage_id=%d" % (poller[ID])
		c.execute(req)
		while not rcv.rstrip().endswith("#scanip#done"):
			...
			if rcv.lstrip().startswith("#state#"):
				...
				print adr_ip
				req = "INSERT INTO sad_results(ip, plage_id) VALUES ('%s', %d)" % (adr_ip, poller[ID])
				c.execute(req)
		req = "UPDATE sad_rangeip SET done = %d WHERE id=%d" % (2, poller[ID])
		c.execute(req)
		s.close()
	except socket.error:
