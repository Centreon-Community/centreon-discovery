 ...
 */
	...
	*/
	function InsertionRelationServiceBDD($host_id,$service_id,$elt_name,$group_description) {
		...
		$host_description = mysql_query("SELECT host_alias FROM host WHERE host_id = ".$host_id." LIMIT 1");
		while ( $data = mysql_fetch_assoc($host_description) ) {
			$host_alias = $data['host_alias'];
		}

		$service_description = $host_alias.'_'.substr($description,9,strlen($description)).'_'.$elt_name;	
