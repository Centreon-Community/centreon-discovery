...
# valid values 
my @valid_types = ("stand","netsc","netsl","as400","cisco","cata","asa","nsc","fg","bc","nokia","hp","lp","hpux");
# CPU OID array
...
# check type  : stand | netsc |  netsl | as400 | cisco | cata | asa | nsc | fg | bc | nokia | hp | lp  | hpux
...
sub print_usage {
    print "Usage: $0 [-v] -H <host> -C <snmp_community> [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>])  [-p <port>] -w <warn level> -c <crit level> -T=[stand|netsl|netsc|as400|cisco|cata|asa|nsc|fg|bc|nokia|hp|lp|hpux] [-f] [-t <timeout>] [-V]\n";
}
...
-C, --community=COMMUNITY NAME
   community name for the host sSNMP agent (implies v1 protocol)
-2, --v2c
   Use snmp v2c
...
-c, --crit=INTEGER | INT,INT,INT
   ...
   3 value check : comma separated level for load or cpu for 1min, 5min, 15min 
-T, --type=stand|netsl|netsc|as400|cisco|cisco|asa|bc|nokia|hp|lp
	CPU check : 
...
sub check_options {
	...
	if (($o_check_type eq "netsl") || ($o_check_type eq "cisco") || ($o_check_type eq "cata") || ($o_check_type eq "asa") || ($o_check_type eq "nsc") || ($o_check_type eq "hpux")) {
